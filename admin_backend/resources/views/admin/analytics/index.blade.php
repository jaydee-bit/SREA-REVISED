...
@section('content')
    ...
    <div class="row row-cards my-3">
        <div class="col-sm-6">
            <div class="card">
                <div class="card-body">
                    <div class="text-muted small mb-1">Median Response Time</div>
                    <div class="h1 mb-0 text-success" id="avgResponseTimeCard">{{ $stats['median_response_time'] }}</div>
        ...
        <div class="col-sm-6">
            <div class="card">
                <div class="card-body">
                    <div class="text-muted small mb-1">Top 5 High Risk Barangays</div>
                    <div class="h1 mb-0 text-danger" id="highRiskCountCard">{{ $stats['top_barangays']['count'] }}
                    </div>
                    <div class="small text-muted" id="highRiskNamesCard">{{ $stats['top_barangays']['names'] }}</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row row-cards mb-3">
        <div class="col-lg-6">
            <div class="card">
                ...
                <div class="card-body">
                    <div id="barangayChartWrap" style="position: relative; height: 220px;">
    ...
    <script>
        ...
        function computeStats(rows) {
            // Median response time — resistant to a single stuck/delayed
            // incident dragging the whole period's number up, unlike a plain mean.
            const withResponse = rows.filter(r => r.response_minutes !== null && r.response_minutes !== undefined);
            const medianMinutes = median(withResponse.map(r => r.response_minutes));
            document.getElementById('avgResponseTimeCard').textContent = medianMinutes !== null ?
                Math.round(medianMinutes) + ' min' :
                'N/A';

            // Top 5 barangays by incident count. The number shown and the names
            // shown are always the same set, so there's no more "4 vs 3 names"
            // or "23 vs 3 names" mismatch.
            const byBarangay = aggregateBarangay(rows);
            const topBarangays = byBarangay.slice(0, 5);

            document.getElementById('highRiskCountCard').textContent = topBarangays.length;
            document.getElementById('highRiskNamesCard').textContent = topBarangays.length ?
                topBarangays.map(b => b.barangay).join(', ') :
        ...
        function renderCharts(rows) {
            ...
            // Give each bar a fixed row height instead of squeezing every
            // barangay into one fixed-height canvas — otherwise, as the number
            // of barangays grows, the bars (and their labels) shrink to the
            // point of overlapping or disappearing.
            const rowHeight = 28;
            const minHeight = 220;
            document.getElementById('barangayChartWrap').style.height =
                Math.max(minHeight, barangayRows.length * rowHeight) + 'px';
