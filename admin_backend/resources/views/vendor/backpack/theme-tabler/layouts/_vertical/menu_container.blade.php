@if (backpack_auth()->check())
    ...
    <aside data-menu-theme="{{ $theme ?? 'system' }}" class="{{ backpack_theme_config('classes.sidebar') ?? 'navbar navbar-vertical'.(($right ?? false) ? ' navbar-right' : '').' navbar-expand-lg navbar-'.($theme ?? 'light') }} @if(backpack_theme_config('options.sidebarFixed')) navbar-fixed @endif" style="background: #F0F2F7 !important; background-image: none !important;">
        <div class="container-fluid">
            ...
            <h1 class="navbar-brand d-none d-lg-block align-self-center mb-3" style="border-bottom: 1px solid #D9DCE3; padding-bottom: 16px; width: 100%; text-align: center;">
                <a class="h2 text-decoration-none mb-0" href="{{ url(backpack_theme_config('home_link')) }}" title="{{ backpack_theme_config('project_name') }}">
                    <img src="{{ asset('images/logo.png') }}" alt="SREA" style="max-height: 60px;">
                </a>
            </h1>
            ...
            <div class="collapse navbar-collapse" id="mobile-menu">
                <ul class="navbar-nav pt-lg-3">
                    ...
                    <div style="border-top: 1px solid #D9DCE3; margin-top: 8px; padding-top: 8px;">
